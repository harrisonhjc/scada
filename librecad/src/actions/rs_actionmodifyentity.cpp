...
{
	RS_Entity* entmp = catchEntity(e);
	if(entmp != NULL){
		
		RS_Image* imgEntity = (RS_Image*)entmp;
		RS_VectorSolutions sol = imgEntity->getCorners();
        QString location = imgEntity->getLocation();
		//qDebug() << "rtu:" << addr;
        graphicView->drawRtuAddress(location ,sol);
		
	}else{
       
        RS_VectorSolutions sol;
        QString addr = "";
        graphicView->drawRtuAddress(addr ,sol);
	}
...
void RS_ActionModifyEntity::mouseReleaseEvent(QMouseEvent* e) {
    if (e->button()==Qt::RightButton) {
        ...
        en = catchEntity(e);
        trigger();
		//qDebug() << "RS_ActionModifyEntity::mouseReleaseEvent";
    }
}
