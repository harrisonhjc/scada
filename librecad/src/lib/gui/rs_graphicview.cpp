    ...
    ,m_bIsCleanUp(false)
{
    ...
    //UPDATE:20190707
    setBackground(QColor(RS_SETTINGS->readEntry("/BackgroundColor", "#FFFFFFFF")));
...
void RS_GraphicView::centerOffsetX() {
    if (container!=NULL && !zoomFrozen) {
		...
		//qDebug() << "RS_GraphicView::centerOffsetX=" << offsetX;
    }
}
...
void RS_GraphicView::centerOffsetY() {
    if (container!=NULL && !zoomFrozen) {
		...
		//qDebug() << "RS_GraphicView::centerOffsetY=" << offsetY;
    }
}
...
void RS_GraphicView::zoomIn(double f, const RS_Vector& center) {
    ...
    zoomWindow(
                ...
                .scale(c, RS_Vector(1.0/f,1.0/f)));
	redraw();
    //qDebug() << "zoomIn:f=" << f << ",center=" << center.x << ":" << center.y;
}
...
void RS_GraphicView::zoomAuto(bool axis, bool keepAspectRatio) {
	...
	//qDebug() << "RS_GraphicView::zoomAuto.container=" << container;
    ...
    if (container!=NULL) {
        ...
        if(true){
			...
			//qDebug() <<"RS_GraphicView::zoomAuto 1:sx:sy=" << sx << ":" << sy;
        } else {
            sx = container->getSize().x;
            sy = container->getSize().y;
			//qDebug() <<"RS_GraphicView::zoomAuto 2:sx:sy=" << sx << ":" << sy;
        }
        ...
        if (sx>RS_TOLERANCE) {
			...
			//qDebug() <<"RS_GraphicView::zoomAuto 2.1:fx=" << fx;
        } else {
        ...
        if (sy>RS_TOLERANCE) {
			...
			//qDebug() <<"RS_GraphicView::zoomAuto 2.1:fy=" << fy;
        } else {
            fFlags += 2; //invalid y factor
        }
        //    std::cout<<"0: fx= "<<fx<<"\tfy="<<fy<<std::endl;

        //qDebug() <<"RS_GraphicView::zoomAuto:fx:fy=" << fx << ":" << fy;
        ...
        //qDebug() <<"RS_GraphicView::zoomAuto 2:fx:fy=" << fx << ":" << fy;
        ...
        //qDebug() <<"RS_GraphicView::zoomAuto 3:fx:fy=" << fx << ":" << fy;
...
//        RS_DEBUG->print("updateGrid");
        //    updateGrid();
        //qDebug() <<"RS_GraphicView::zoomAuto 1:offx:offfy=" << offsetX << ":" << offsetY;
		offsetX = 140;//-34;
		offsetY = 0;//-251;
        redraw();
		//qDebug() <<"RS_GraphicView::zoomAuto 2:offx:offfy=" << offsetX << ":" << offsetY;
		//qDebug() <<"RS_GraphicView::zoomAuto:fx:fy=" << fx << ":" << fy;
    }
...
void RS_GraphicView::drawLayer1(RS_Painter *painter) {
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawLayer1.");
...
void RS_GraphicView::drawLayer2(RS_Painter *painter,double color)
{
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawLayer2.");
	//harrison
	//4.0:red 5.0:green 6.0:Blue 
	//double patternOffset(4.0); 
	drawEntity(painter, container,color);	//	Draw all entities.
...
void RS_GraphicView::drawLayer3(RS_Painter *painter) {
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawLayer3.");
    // drawing zero points:
    if (!isPrintPreview()) {
	...
	//draw jpg
	/*
    RS_Vector scale = RS_Vector(0.1,0.1);
    double angle = 0.0;
	RS_Vector insertionPoint = RS_Vector(0,0);
    QImage img = QImage("E:/tmp/A1-2F.jpg");
	painter->drawImg(img,
	               toGui(insertionPoint),
	               angle,
	               scale);
	               */
}
/*
void RS_GraphicView::drawLayer4(RS_Painter *painter, QStringList&  alertRTUs)
{
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawLayer4.");

	double patternOffset(1.0);
	container->setRtuList(alertRTUs);
    drawEntity(painter, container, patternOffset);	//	Draw all entities.
    //	If not in print preview, draw the absolute zero reference.
    //	----------------------------------------------------------
    if (!isPrintPreview())
        drawAbsoluteZero(painter);
...
void RS_GraphicView::drawLayer5(RS_Painter *painter, QStringList&  alertRTUs)
{
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawLayer4.");

	double patternOffset(21.0); //R4 alert
	container->setRtuList(alertRTUs);
    drawEntity(painter, container, patternOffset);	//	Draw all entities.
    //	If not in print preview, draw the absolute zero reference.
    //	----------------------------------------------------------
...
void RS_GraphicView::drawLayer6(RS_Painter *painter, QStringList&  alertRTUs)
{
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawLayer4.");

	double patternOffset(31.0); //Fault alert
	container->setRtuList(alertRTUs);
    drawEntity(painter, container, patternOffset);	//	Draw all entities.
    //	If not in print preview, draw the absolute zero reference.
    //	----------------------------------------------------------
...
*/
void RS_GraphicView::drawLayer7(RS_Painter *painter, QMap<QString,double>&  alertRTUs)
{
	double patternOffset(71.0); //all alerts
	container->setRtuList(alertRTUs);
    drawEntity(painter, container, patternOffset);	//	Draw all entities.
    //	If not in print preview, draw the absolute zero reference.
    //	----------------------------------------------------------
...
void RS_GraphicView::drawLayer8(RS_Painter *painter, QMap<QString,double>&  alertRTUs)
{
	double patternOffset(91.0); //fire alerts
	container->setRtuList(alertRTUs);
    drawEntity(painter, container, patternOffset);	//	Draw all entities.
    //	If not in print preview, draw the absolute zero reference.
    //	----------------------------------------------------------
...
void RS_GraphicView::drawEntity(RS_Painter *painter, RS_Entity* e) {
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawEntity 1.");
    double offset(5.0);
    drawEntity(painter,e,offset);
}
void RS_GraphicView::drawEntity(RS_Painter *painter, RS_Entity* e, double& patternOffset) {

	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawEntity 2:offset=%f.",patternOffset);
    // update is diabled:
	...
	if(patternOffset == 21.0){
		//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawEntity 21:offset=%f",patternOffset);
		drawEntityPlain(painter, e, patternOffset);
		return;
	}else
	if(patternOffset == 31.0){
		//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawEntity 21:offset=%f",patternOffset);
		drawEntityPlain(painter, e, patternOffset);
		return;
	}
    ...
    if (isDraftMode()) {
		...
		//	qDebug() << "drawEntity:patternOffset=" << patternOffset;
...
//harrison
void RS_GraphicView::drawEntityR4(RS_Painter *painter, RS_Entity* e, const QString& address) {

	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawEntity addr R4.");
    // update is diabled:
    // given entity is NULL:
    if (e==NULL) {
        return;
	...
	drawEntityPlainR4(painter, e, address);
	return;
	
}
void RS_GraphicView::drawEntityPlainR4(RS_Painter *painter, RS_Entity* e, const QString& address) 
{
	
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawEntityPlain addr R4.");
	...
	//if (e->rtti()==RS2::EntityImage)
    //    qDebug() << "drawEntityPlain:address";
		
	RS_Image* m = (RS_Image*)e;
	m->drawAlertR4(painter, this, address);

}

void RS_GraphicView::drawEntityFault(RS_Painter *painter, RS_Entity* e, const QString& address) {
	...
	drawEntityPlainFault(painter, e, address);
	return;
	
}
void RS_GraphicView::drawEntityPlainFault(RS_Painter *painter, RS_Entity* e, const QString& address) 
{
	...
	RS_Image* m = (RS_Image*)e;
	m->drawAlertFault(painter, this, address);

}

void RS_GraphicView::drawEntity(RS_Painter *painter, RS_Entity* e, const QString& address) {

	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawEntity addr.");
    // update is diabled:
    // given entity is NULL:
    if (e==NULL) {
        return;
	...
	drawEntityPlain(painter, e, address);
	return;
	
}

void RS_GraphicView::drawEntityPlain(RS_Painter *painter, RS_Entity* e, const QString& address) 
{
	
	//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_GraphicView::drawEntityPlain addr.");
	...
	RS_Image* m = (RS_Image*)e;
	m->drawAlarm(painter, this, address);

}


void RS_GraphicView::drawEntityAlert(RS_Painter *painter, RS_Entity* e, const QString& address,double& patternOffset) {
	...
	drawEntityPlainAlert(painter, e, address,patternOffset);
	return;
	
}

void RS_GraphicView::drawEntityPlainAlert(RS_Painter *painter, RS_Entity* e, const QString& address,double& patternOffset) 
{
	...
	RS_Image* m = (RS_Image*)e;
	m->drawAlert(painter, this, address,patternOffset);
...
void RS_GraphicView::drawEntityPlain(RS_Painter *painter, RS_Entity* e, double& patternOffset) {
	//RS_DEBUG->print("RS_GraphicView::drawEntityPlain 1.");
	...
	if (e->rtti()==RS2::EntityImage){
        //qDebug() << "drawEntityPlain 1.1:patternOffset=" << patternOffset;
	...
	//if (e->rtti()==RS2::EntityImage)
    //    qDebug() << "drawEntityPlain:patternOffset=" << patternOffset;
		
	e->draw(painter, this, patternOffset);

}
void RS_GraphicView::drawEntityPlain(RS_Painter *painter, RS_Entity* e) {
	//RS_DEBUG->print("RS_GraphicView::drawEntityPlain 2.");
    if (e==NULL) {
        return;
    }
	...
	//if (e->rtti()==RS2::EntityImage)
	//	qDebug() << "drawEntityPlain:patternOffset=0";
...
void RS_GraphicView::drawPaper(RS_Painter *painter) {
    ...
    posPaper = v1;
    sizePaper = v2 - v1;
    //RS_DEBUG->print(RS_Debug::D_WARNING, "RS_GraphicView::drawPaper:scale=%lf",scale);
    //RS_DEBUG->print(RS_Debug::D_WARNING, "RS_GraphicView::drawPaper:factor=%lf:%lf",factor.x, factor.y);
    //RS_DEBUG->print(RS_Debug::D_WARNING, "RS_GraphicView::drawPaper:pinsbase=%lf,%lf",pinsbase.x, pinsbase.y);
    //RS_DEBUG->print(RS_Debug::D_WARNING, "RS_GraphicView::drawPaper:size=%lf,%lf",size.x , size.y);
    //RS_DEBUG->print(RS_Debug::D_WARNING, "RS_GraphicView::drawPaper:v1=%lf,%lf",v1.x, v1.y);
    //RS_DEBUG->print(RS_Debug::D_WARNING, "RS_GraphicView::drawPaper:v2=%lf,%lf",v2.x, v2.y);

    // gray background:
    //UPDATE:20190707
    /*
    if(isPrinting())
		painter->fillRect(0,0, getWidth(), getHeight(),RS_Color(255,255,255));
	else
		painter->fillRect(0,0, getWidth(), getHeight(),RS_Color(0,0,0));
    */
    painter->fillRect(0,0, getWidth(), getHeight(),RS_Color(255,255,255));
    ...
    double angle = 0.0;
	RS_Vector uVector = RS_Vector(1,0);;
	RS_Vector vVector = RS_Vector(0,1);;
    RS_Vector scale2 = RS_Vector(toGuiDX(uVector.magnitude()),
                               toGuiDY(vVector.magnitude()));

	//draw floor map
	RS_Vector insertionPoint = RS_Vector(0,0);
    QImage img = QImage(wallPaperFileName);

	if(isPrinting())
		img.invertPixels(QImage::InvertRgb);
	
    painter->drawImg(img, v1, angle, scale2);

    
	//draw rtu address
    if(imageAddress != NULL && imageAddress.length() > 0){
        RS_Vector v0 = toGui(imageSol.get(0));
        RS_Vector v2 = toGui(imageSol.get(2));
		int width = imageAddress.length()*20;
        painter->setPen(QColor(255,255,255));
        painter->fillRect(v0.x ,v0.y ,width, 30 ,RS_Color(255,255,0));
        painter->fillRect(v0.x ,v0.y+3 ,width, 27 ,RS_Color(0,102,204));
        painter->drawTextH(v0.x ,v0.y+3 ,width, 27, imageAddress);
	}
	//qDebug() << "RS_GraphicView::drawPaper:uVector=" << toGuiDX(uVector.magnitude()) << ":" << toGuiDX(uVector.magnitude()) ;
	//qDebug() << "RS_GraphicView::drawPaper:vVector=" << toGuiDX(vVector.magnitude()) << ":" << toGuiDX(vVector.magnitude()) ;
    
}

void RS_GraphicView::drawRtuAddress(QString& address,RS_VectorSolutions& sol)
{
	//qDebug() << "RS_GraphicView::drawRtuAddress:";
