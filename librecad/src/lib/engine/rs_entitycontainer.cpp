...
void RS_EntityContainer::draw(RS_Painter* painter, RS_GraphicView* view,
	...
	//draw image only : harrison
	////////////////////////////////////////////////
	RS_Image *m;
	/*
	if(patternOffset == 1.0){ //green
		for(RS_Entity* e=firstEntity(RS2::ResolveNone);
	         e!=NULL;
	         e = nextEntity(RS2::ResolveNone)){
			 	if(e->rtti() == RS2::EntityImage){
					m = (RS_Image*)e;
					//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_EntityContainer::draw:image:%s.",m->data.file.toLatin1().data());
					
					for (int i = 0; i < rtuList.size(); ++i){
						if(m->data.rtuAddress == rtuList.at(i)){
							//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_EntityContainer::draw:rtu 1.0.");
							view->drawEntity(painter, e, rtuList.at(i));
						}
					}
		...
		for(RS_Entity* e=firstEntity(RS2::ResolveNone);
	         ...
	         e = nextEntity(RS2::ResolveNone)){
			 	if(e->rtti() == RS2::EntityImage){
					...
					for (int i = 0; i < rtuList.size(); ++i){
						if(m->data.rtuAddress == rtuList.at(i)){
							//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_EntityContainer::draw:rtu 21.0.");
							view->drawEntityR4(painter, e, rtuList.at(i));
						}
					}
	         ...
	         e = nextEntity(RS2::ResolveNone)){
			 	if(e->rtti() == RS2::EntityImage){
					...
					for (int i = 0; i < rtuList.size(); ++i){
						if(m->data.rtuAddress == rtuList.at(i)){
							//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_EntityContainer::draw:rtu 21.0.");
							view->drawEntityFault(painter, e, rtuList.at(i));
						}
					}
	...
	if(patternOffset == 71.0){ //all alerts
		for(RS_Entity* e=firstEntity(RS2::ResolveNone);
			 e!=NULL;
	         e = nextEntity(RS2::ResolveNone)){
			 	if(e->rtti() == RS2::EntityImage){
					m = (RS_Image*)e;
					//RS_DEBUG->print(RS_Debug::D_WARNING,"RS_EntityContainer::draw:image:%s.",m->data.file.toLatin1().data());
					if(rtuList.contains(m->data.rtuAddress)){
						view->drawEntityAlert(painter, e, m->data.rtuAddress,rtuList[m->data.rtuAddress]);
					}
			 	}
	     }
		return;
	}else
	if(patternOffset == 91.0){ //print fire alerts
		for(RS_Entity* e=firstEntity(RS2::ResolveNone);
			 e!=NULL;
	         e = nextEntity(RS2::ResolveNone)){
			 	if(e->rtti() == RS2::EntityImage){
					m = (RS_Image*)e;
                    //qDebug() << "RS_EntityContainer::draw:91.0.";
					if(rtuList.contains(m->data.rtuAddress)){
						
						view->drawEntityAlert(painter, e, m->data.rtuAddress,patternOffset); //red
					}
			 	}
	     }
		return;
...
//harrison
void RS_EntityContainer::setRtuList(QMap<QString,double>&  alertRTUs){
	rtuList = alertRTUs;
}
