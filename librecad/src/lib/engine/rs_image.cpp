...
void RS_Image::update() {
    ...
    //qDebug() << "RS_Image::update:";

    // the whole image:
    //QImage image = QImage(data.file);
    QString fnGreen = data.file;
	QString fnBlue = data.file;
	QString fnMagenta = data.file;
	int index = fnGreen.indexOf(QChar('.'));
	fnGreen.replace(index,4,"g.png");
	fnBlue.replace(index,4,"b.png");
	fnMagenta.replace(index,4,"p.png");

	img = QImage(data.file);
    if(!img.isNull()) {
        data.size = RS_Vector(img.width(), img.height());
    }
	imgGreen = QImage(fnGreen);
    if(!imgGreen.isNull()) {
        data.size = RS_Vector(imgGreen.width(), imgGreen.height());
    }
	imgBlue = QImage(fnBlue);
    if(!imgBlue.isNull()) {
        data.size = RS_Vector(imgBlue.width(), imgBlue.height());
    }
	imgMagenta = QImage(fnMagenta);
    if(!imgMagenta.isNull()) {
        data.size = RS_Vector(imgMagenta.width(), imgMagenta.height());
    }
...
void RS_Image::drawAlert(RS_Painter* painter, RS_GraphicView* view, const QString& address,double& patternOffset) {
    if (painter==NULL || view==NULL || img.isNull()) {
        return;
    }
	//qDebug() << "RS_Image::drawAlert:" << patternOffset;
	
	RS_Vector scale2 = RS_Vector(view->toGuiDX(data.uVector.magnitude()),
                                view->toGuiDY(data.vVector.magnitude()));
    double angle = data.uVector.angle();

	if(patternOffset == 91.0){
		painter->drawImg(img,
	...
	if(toggleAlarm == 1){
		...
		if(patternOffset == 6.0){ //blue
			if(imgBlue.isNull())
				return;
			painter->drawImg(imgBlue,
		                   view->toGui(data.insertionPoint),
		                   angle, scale2);
			imgColor = double(6.0);
		}else
		if(patternOffset == 7.0){ //pink
			if(imgMagenta.isNull())
				return;
			painter->drawImg(imgMagenta,
		                   view->toGui(data.insertionPoint),
		                   angle, scale2);
			imgColor = double(7.0);
		}
		toggleAlarm = 0;
		//qDebug() << "RS_Image::drawAlarm 1";
	}else{
        if(imgGreen.isNull())
				return;
		painter->drawImg(imgGreen,
	                    view->toGui(data.insertionPoint),
	                    angle, scale2);
		toggleAlarm = 1;
		//qDebug() << "RS_Image::drawAlarm 0";
	}
...
void RS_Image::drawAlarm(RS_Painter* painter, RS_GraphicView* view, const QString& address) {
    if (painter==NULL || view==NULL || img.isNull()) {
        return;
    }
	qDebug() << "RS_Image::drawAlarm:";
...
void RS_Image::drawAlertR4(RS_Painter* painter, RS_GraphicView* view, const QString& address) {
    if (painter==NULL || view==NULL || img.isNull()) {
        return;
    }
	qDebug() << "RS_Image::drawAlarmR4.";
...
void RS_Image::drawAlertFault(RS_Painter* painter, RS_GraphicView* view, const QString& address) {
    if (painter==NULL || view==NULL || img.isNull()) {
        return;
    }
	qDebug() << "RS_Image::drawAlarmFault.";
	...
	if(toggleAlarm == 1){
		painter->drawImg(imgGreen,
	                    ...
	                    angle, scale2);
		toggleAlarm = 0;
		//qDebug() << "RS_Image::drawAlertFault 1";
	}else{
		...
		painter->drawImg(imgMagenta,
	                    ...
	                    angle, scale2);
		toggleAlarm = 1;
		//qDebug() << "RS_Image::drawAlertFault 0";
	}
...
void RS_Image::draw(RS_Painter* painter, RS_GraphicView* view, double& patternOffset) {
    ...
    //qDebug() << "RS_Image::draw:uVector=" << data.uVector.x << ":" << data.uVector.y ;
	//qDebug() << "RS_Image::draw:vVector=" << data.vVector.x << ":" << data.vVector.y ;
