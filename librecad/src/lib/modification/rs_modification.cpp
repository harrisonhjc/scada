    ...
    for (RS_Entity* e=((RS_EntityContainer*)source)->firstEntity();
        ...
        if (e!=NULL) {
			...
			if(e->rtti() == RS2::EntityImage){
				...
				SensorDlg* dlg = new SensorDlg();
				dlg->exec();
				m->data.file = dlg->getSensorFn();
				/*
				//qDebug() << "RS_Modification::paste:fn+:" << m->data.file ;
				if(m->data.file.contains("sensor5")){
					m->data.file.replace("sensor5","sensor6");
				}else
				if(m->data.file.contains("sensor6")){
					m->data.file.replace("sensor6","sensor5");
				}else
				if(m->data.file.contains("sensor7")){
					m->data.file.replace("sensor7","sensor8");
				}else
				if(m->data.file.contains("sensor8")){
					m->data.file.replace("sensor8","sensor7");
				}else
				if(m->data.file.contains("sensor9")){
					m->data.file.replace("sensor9","sensor10");
				}else
				if(m->data.file.contains("sensor10")){
					m->data.file.replace("sensor10","sensor9");
				}else
				if(m->data.file.contains("sensor11")){
					m->data.file.replace("sensor11","sensor12");
				}else
				if(m->data.file.contains("sensor12")){
					m->data.file.replace("sensor12","sensor11");
				}else
				if(m->data.file.contains("sensor27")){
					m->data.file.replace("sensor27","sensor28");
				}else
				if(m->data.file.contains("sensor28")){
					m->data.file.replace("sensor28","sensor27");
				}else
				if(m->data.file.contains("sensor29")){
					m->data.file.replace("sensor29","sensor30");
				}else
				if(m->data.file.contains("sensor30")){
					m->data.file.replace("sensor30","sensor29");
				}
				//qDebug() << "RS_Modification::paste:fn-:" << m->data.file ;
				*/
			}
