...
bool RS_FilterDXFRW::fileImport(RS_Graphic& g, const QString& file, RS2::FormatType type) {
	...
	//read image data
	/////////////////////////////////////////////////////////
    if(!file.contains("ansi31") &&
       !file.contains("grass")){

        QString imgEntityFileName = file;
        int len = file.size();
        imgEntityFileName.replace(len-3,3,"img");
        dxfRW dxfRead(QFile::encodeName(imgEntityFileName));
        bool success = dxfRead.readRtuImages(this);
        if (success==false){
            RS_DEBUG->print(RS_Debug::D_WARNING,
                           "Cannot open img file '%s'.", (const char*)QFile::encodeName(imgEntityFileName));
            return false;
        }
    }
	///////////////////////////////////////////////////////////////////////////////////
...
void RS_FilterDXFRW::addImage(const DRW_Image *data) {
	...
	image->setLocation(QString::fromStdString(data->location));
	image->setRtuAddress(QString::fromStdString(data->rtuAddress));
	rtuMap.insert(QString::fromStdString(data->rtuAddress),
		         QString::fromStdString(data->location));
...
bool RS_FilterDXFRW::fileExport(RS_Graphic& g, const QString& file, RS2::FormatType type)
{
	...
	int len = file.size();
	imgEntityFileName.replace(len-3,3,"img");
	dxfW = new dxfRW(QFile::encodeName(imgEntityFileName));
	bool success = dxfW->writeRtuImages(this);
	delete dxfW;
...
void RS_FilterDXFRW::writeImageEntities(){
    for (RS_Entity *e = graphic->firstEntity(RS2::ResolveNone);
         e != NULL; e = graphic->nextEntity(RS2::ResolveNone)) {
        if (e->rtti()==RS2::EntityImage &&
            !e->getFlag(RS2::FlagUndone) ) {
...
void RS_FilterDXFRW::writeImage(RS_Image * i) {
	...
	//harrison
    image.location = i->getLocation().toStdString();
	image.rtuAddress = i->getRtuAddress().toStdString();
...
void RS_FilterDXFRW::writeRtuImage(RS_Image * i,dxfWriter *writer)
{
    DRW_Image image;
	...
	//write entity
	image.handle = ++entCount;
    writer->writeString(100, "AcDbEntity");
    if (image.space == 1)
        writer->writeInt16(67, 1);

	writer->writeUtf8String(8, image.layer);
    writer->writeUtf8String(6, image.lineType);
    writer->writeInt16(62, image.color);
    writer->writeInt16(370, DRW_LW_Conv::lineWidth2dxfInt(image.lWeight));
    ///
   	
    writer->writeString(100, "AcDbRasterImage");
    writer->writeUtf8String(102, image.location);
    writer->writeUtf8String(103, image.rtuAddress);
	//harrison
	/*
	std::string t = encoder.fromUtf8(ent->location);
    rtuOFilestream << 102 << std::endl << t << std::endl ;
	t = encoder.fromUtf8(ent->rtuAddress);
    rtuOFilestream << 103 << std::endl << t << std::endl ;
	*/
    writer->writeDouble(10, image.basePoint.x);
    writer->writeDouble(20, image.basePoint.y);
    writer->writeDouble(30, image.basePoint.z);
    writer->writeDouble(11, image.secPoint.x);
    writer->writeDouble(21, image.secPoint.y);
    writer->writeDouble(31, image.secPoint.z);
    writer->writeDouble(12, image.vx);
    writer->writeDouble(22, image.vy);
    writer->writeDouble(32, image.vz);
    writer->writeDouble(13, image.sizeu);
    writer->writeDouble(23, image.sizev);
    writer->writeInt16(340, image.handle);
    writer->writeInt16(70, 1);
    writer->writeInt16(280, image.clip);
    writer->writeInt16(281, image.brightness);
    writer->writeInt16(282, image.contrast);
    writer->writeInt16(283, image.fade);
