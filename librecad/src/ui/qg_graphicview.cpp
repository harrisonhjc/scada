...
QG_GraphicView::QG_GraphicView(QWidget* parent, const char* name, Qt::WindowFlags f)
    ...
    PixmapLayer1=PixmapLayer2=PixmapLayer3=PixmapLayer4=PixmapLayer5=PixmapLayer6=PixmapLayer7=NULL;
...
void QG_GraphicView::zoomInMap(RS_Vector v) {

    setCurrentAction(new RS_ActionZoomIn(*container, *this, RS2::In, RS2::Both, v));
}

void QG_GraphicView::zoomOutMap(RS_Vector v) {

    setCurrentAction(new RS_ActionZoomIn(*container, *this, RS2::Out, RS2::Both, v));
}
...
void QG_GraphicView::paintEvent(QPaintEvent *) {
	...
	PixmapLayer4 = getPixmapForView(PixmapLayer4);
	PixmapLayer5 = getPixmapForView(PixmapLayer5);
	PixmapLayer6 = getPixmapForView(PixmapLayer6);
	PixmapLayer7 = getPixmapForView(PixmapLayer7);
    ...
    if (redrawMethod & RS2::RedrawDrawing) {
        ...
        drawLayer2((RS_Painter*)&painter2,colorImage);
	...
	/*
	//draw alarm
	if(alertType == (int)RS2::RedrawAlarm) {
		
	    PixmapLayer4->fill(Qt::transparent);
	    RS_PainterQt painter4(PixmapLayer4);
	    drawLayer4((RS_Painter*)&painter4, rtuList);
	    painter4.end();
		//qDebug() << "QG_GraphicView::paintEvent:RedrawAlarm";
        
    }else	
	if(alertType == (int)RS2::RedrawR4){
		
	    PixmapLayer5->fill(Qt::transparent);
	    RS_PainterQt painter5(PixmapLayer5);
	    drawLayer5((RS_Painter*)&painter5, rtuList);
	    painter5.end();
		//qDebug() << "QG_GraphicView::paintEvent:RedrawR4";
        
    }else	
	if(alertType == (int)RS2::RedrawFault){
		
	    PixmapLayer6->fill(Qt::transparent);
	    RS_PainterQt painter6(PixmapLayer6);
	    drawLayer6((RS_Painter*)&painter6, rtuList);
	    painter6.end();
		//qDebug() << "QG_GraphicView::paintEvent:RedrawFault";
        
    }else	*/
	if(alertType == (int)RS2::RedrawAlert){
		
	    PixmapLayer7->fill(Qt::transparent);
	    RS_PainterQt painter7(PixmapLayer7);
	    drawLayer7((RS_Painter*)&painter7, rtuList);
	    painter7.end();
		//qDebug() << "QG_GraphicView::paintEvent:RedrawFault";
	...
	/*
	if(alertType == (int)RS2::RedrawAlarm){
		wPainter.drawPixmap(0,0,*PixmapLayer4);
	}else
	if(alertType == (int)RS2::RedrawR4){
		wPainter.drawPixmap(0,0,*PixmapLayer5);
	}else
	if(alertType == (int)RS2::RedrawFault){
		wPainter.drawPixmap(0,0,*PixmapLayer6);
	}*/
	if(alertType == (int)RS2::RedrawAlert){
		wPainter.drawPixmap(0,0,*PixmapLayer7);
	}
	wPainter.end();
...
void QG_GraphicView::renderToImage(QImage& image, QSize& mapSize)
{
    //RS_DEBUG->print(RS_Debug::D_WARNING, "QG_GraphicView::renderToImage ++");
    ...
    QRect mapBoundingRect = QRect(0,0,mapSize.width(), mapSize.height());
    ...
    RS_PainterQt wPainter(&image);
    
    //wPainter.translate(centerOffset);
    wPainter.scale(scale, scale);
    //wPainter.translate(-mapBoundingRect.topLeft());
...
//harrison
void QG_GraphicView::setRtuList(QMap<QString,double>&  alertRTUs){
	rtuList = alertRTUs;
	
}

void QG_GraphicView::setImageColor(double color){
	colorImage = color;
	redraw();
}

void QG_GraphicView::restoreRedrawMethod()
{
