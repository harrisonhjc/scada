...
public:
    void zoomInMap(RS_Vector v);
    void zoomOutMap(RS_Vector v); 

	int alertType;
	double colorImage;
	QMap<QString,double> rtuList;
    void setRtuList(QMap<QString,double>&  alertRTUs);
	void setImageColor(double color);
	void restoreRedrawMethod();
	...
	//virtual void redrawImg(RS2::RedrawMethod method=RS2::RedrawAll);
    ...
    void renderToImage(QImage& image, QSize& mapSize);
...
protected:
	...
	//harrison
	QPixmap *PixmapLayer4;  // Used for alarm
	QPixmap *PixmapLayer5;  //draw R4
	QPixmap *PixmapLayer6;  //draw fault	
	QPixmap *PixmapLayer7;  //draw fault	
