...
bool QC_MDIWindow::slotFileOpen(const QString& fileName, RS2::FormatType type) {
    ...
    if (document!=NULL && !fileName.isEmpty()) {
        ...
        graphicView->zoomIn(1.0);
		graphicView->setWallPaper(fileName);
        ...
        if (ret) {
            ...
            if (fileName.endsWith(".lff") || fileName.endsWith(".cxf")) {
                ...
                mMiniMapDock->setMapDocument(this);
                mMiniMapDock->setFloating(false);
                mMiniMapDock->setFeatures(QDockWidget::NoDockWidgetFeatures);
                mMiniMapDock->setMaximumHeight(230);
                mMiniMapDock->setMaximumWidth(251);
                mMiniMapDock->setMinimumHeight(230);
                mMiniMapDock->setMinimumWidth(251);
                //QRect rec = QApplication::desktop()->screenGeometry();
                //mMiniMapDock->move(rec.width()-251-20 ,20);
...
void QC_MDIWindow::slotAlarm(){
	
	graphicView->redraw(RS2::RedrawAlert);
	
}

void QC_MDIWindow::slotAlarmR4(){
	
	graphicView->redraw(RS2::RedrawR4);
	
}

void QC_MDIWindow::slotAlarmFault(){
	
	graphicView->redraw(RS2::RedrawFault);
...
void QC_MDIWindow::enableAlarm(QMap<QString,double>&  alertRTUs){

	//RS_DEBUG->print(RS_Debug::D_WARNING,"QC_MDIWindow::enableAlarm.");
	if(alarmTimer != NULL){
		//RS_DEBUG->print(RS_Debug::D_WARNING,"QC_MDIWindow::enableAlarm.timer.");
		connect(alarmTimer, SIGNAL(timeout()), this, SLOT(slotAlarm()));
		graphicView->setRtuList(alertRTUs);
		double color(5.0);
		setImageColor(color);
		alarmTimer->stop();
...
void QC_MDIWindow::disableAlarm(){
	if(alarmTimer != NULL){
		alarmTimer->stop();
		double color(5.0);
		setImageColor(color);
		graphicView->restoreRedrawMethod();
		
	}
	
}
/*
void QC_MDIWindow::enableAlarmR4(QStringList&  alertRTUs){

	//RS_DEBUG->print(RS_Debug::D_WARNING,"QC_MDIWindow::enableAlarm R4.");
	if(alarmTimer != NULL){
		//RS_DEBUG->print(RS_Debug::D_WARNING,"QC_MDIWindow::enableAlarm.timer R4.");
		connect(alarmTimer, SIGNAL(timeout()), this, SLOT(slotAlarmR4()));
		graphicView->setRtuList(alertRTUs);
		double color(5.0);
		setImageColor(color);
		alarmTimer->stop();
...
void QC_MDIWindow::disableAlarmR4(){
	if(alarmTimer != NULL){
		alarmTimer->stop();
		double color(5.0);
		setImageColor(color);
...
void QC_MDIWindow::enableAlarmFault(QStringList&  alertRTUs){

	//RS_DEBUG->print(RS_Debug::D_WARNING,"QC_MDIWindow::enableAlarm R4.");
	if(alarmTimer != NULL){
		//RS_DEBUG->print(RS_Debug::D_WARNING,"QC_MDIWindow::enableAlarm.timer R4.");
		connect(alarmTimer, SIGNAL(timeout()), this, SLOT(slotAlarmFault()));
		graphicView->setRtuList(alertRTUs);
		double color(5.0);
		setImageColor(color);
		alarmTimer->stop();
...
void QC_MDIWindow::disableAlarmFault(){
	if(alarmTimer != NULL){
		alarmTimer->stop();
		double color(5.0);
		setImageColor(color);
...
void QC_MDIWindow::setImageColor(double color){
	
	if(graphicView != NULL)
		graphicView->setImageColor(color);
