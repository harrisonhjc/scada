...
#ifndef QC_APP_ICON
# define QC_APP_ICON ":/main/scada.png"
#endif
          ...
          m_qDraftModeTitle(" ["+tr("Draft Mode")+"]")
{
	...
	sensorPrefixPath = ":/actions/sensor";
	sensorSuffixPath = ".png";
...
# endif
	...
	//harrison
	//if (cadToolBar!=NULL) {
	//	((QToolBar *)(cadToolBar->parentWidget()))->hide();
    //}
}
...
void QC_ApplicationWindow::closeEvent(QCloseEvent* ce) {
    ...
    qDebug() << "QC_ApplicationWindow::closeEvent(): OK";
    
	if(svModeStatus == true){
	    ...
	    for (int i=0; i<windows.size();i++) {
			...
			if(w != NULL){
				...
				QC_MDIWindow* qw;
				qw = qobject_cast<QC_MDIWindow*>(w->widget());
				openedFiles.removeAll(qw->getDocument()->getFilename());
				//qw->setFourceClose();
                qw->close();
	    ...
	    */
    	...
    	mdiAreaCAD->activeSubWindow()->close();
	...
	//svWindow->notifyMapWindowClosed();
	//harrison
    //emit qc_AppWindowlose();
...
void QC_ApplicationWindow::mousePressEvent(QMouseEvent * e){
	if(e->button()==Qt::RightButton)
        e->ignore();
}

bool QC_ApplicationWindow::eventFilter(QObject* object, QEvent* event) {
...
void QC_ApplicationWindow::initMDI() {
    ...
    vbMDI = new QFrame(this);
    vbMDI->setSizePolicy(QSizePolicy::Maximum,QSizePolicy::Maximum);
    QRect screenGeometry = QApplication::desktop()->availableGeometry();
    //vbMDI->setFixedSize(screenGeometry.width() - 280, screenGeometry.height()-100);
    //vbMDI->setMinimumWidth(screenGeometry.width() - 280);
    //vbMDI->setMinimumHeight(screenGeometry.height());
    QVBoxLayout *layout = new QVBoxLayout;
    //vbMDI->setFrameStyle(QFrame::StyledPanel | QFrame::Sunken);
    //vbMDI->setFrameStyle(QFrame::NoFrame);
    layout->setContentsMargins ( 0, 0, 0, 0 );
    mdiAreaCAD = new QMdiArea(this);
    mdiAreaCAD->setMinimumWidth(screenGeometry.width() - DOCK_WIDTH);
    mdiAreaCAD->setMinimumHeight(screenGeometry.height() - 100);
...
#endif
    vbMDI->setLayout(layout);
    setCentralWidget(vbMDI);
...
void QC_ApplicationWindow::initActions(void)
{
	...
	*/
    ...
    gridAction=actionFactory.addGUI(menu, zoomToolBar, this, RS2::ActionViewGrid);
    connect(this, SIGNAL(gridChanged(bool)), gridAction, SLOT(setChecked(bool)));
...
//            connect(this, SIGNAL(windowsChanged(bool)), a, SLOT(setEnabled(bool)));
	...
	*/
	...
	//modify toolbar
	modifyToolBar->setWindowTitle(tr("MODIFY"));
	modifyAction = new QAction(tr("Modify"), this);
	modifyAction->setIcon(QIcon(":/actions/modifyentity.png")); 
	connect(modifyAction, SIGNAL(triggered()), actionHandler, SLOT(slotModifyEntity()));
	modifyToolBar->addAction(modifyAction);
	//modify::del action
	delAction = new QAction(tr("&Delete selected"), this);
    delAction->setIcon(QIcon(":/actions/modifydelete.png"));
    delAction->setShortcut(QKeySequence::Delete);
	connect(delAction, SIGNAL(triggered()), actionHandler, SLOT(slotModifyDeleteQuick()));
	modifyToolBar->addAction(delAction);
	...
	for(int i=0;i<45;i++){
		QString index = QString::number(i+1, 10);  
		sensorPath = sensorPrefixPath + index + sensorSuffixPath;
		sensorActions[i] = new QAction("Sensor"+index, this);
		sensorActions[i]->setIcon(QIcon(sensorPath)); 
		signalMapper->setMapping(sensorActions[i],i+1) ;
       	connect(sensorActions[i], SIGNAL(triggered()), signalMapper, SLOT(map())) ;
		if(i<23)
			sensorToolBar->addAction(sensorActions[i]);
		else
			sensor2ToolBar->addAction(sensorActions[i]);
		
	}
    connect (signalMapper, SIGNAL(mapped(int)), this, SLOT(slotSensor(int))) ;
	addToolBar(Qt::RightToolBarArea, sensorToolBar);
	addToolBarBreak(Qt::RightToolBarArea);
	addToolBar(Qt::RightToolBarArea, sensor2ToolBar);
	//addToolBarBreak(Qt::RightToolBarArea);
	*/
    //////////////////////////////
    QString sensorPath;
    ...
    for(int i=0;i<NUM_SENSOR;i++){
        ...
        signalMapper->setMapping(sensorActions[i],i+1) ;
        connect(sensorActions[i], SIGNAL(triggered()), signalMapper, SLOT(map())) ;
        sensorToolBar[i/(NUM_SENSOR/NUM_SENSORTOOLBAR)]->addAction(sensorActions[i]);
    }
    connect (signalMapper, SIGNAL(mapped(int)), this, SLOT(slotSensor(int))) ;
    /////////////////////////////////////

	cadPToolBar->installEventFilter(this);
    
    for(int i=0;i<NUM_SENSORTOOLBAR;i++){
        sensorToolBar[i]->installEventFilter(this);
    }
	
	
	modifyToolBar->installEventFilter(this);
	fileToolBar->installEventFilter(this);
	editToolBar->installEventFilter(this);
	zoomToolBar->installEventFilter(this);
...
void QC_ApplicationWindow::setGridMode(bool mode)
{
	gridAction->setChecked(mode);
}
...
void QC_ApplicationWindow::initToolBar() {
	...
	modifyToolBar = new QToolBar( "Modify Operations", this);
    modifyToolBar->setSizePolicy(toolBarPolicy);
    modifyToolBar->setObjectName ( "ModifyTB" );
	/*
...
*/
    ...
    t->setFixedWidth(0); //66
    t->setFloatable(false);
    //t->setAllowedAreas(Qt::LeftToolBarArea | Qt::RightToolBarArea);
    //addToolBar(Qt::LeftToolBarArea, t);
	t->setAllowedAreas(Qt::RightToolBarArea);
    addToolBar(Qt::RightToolBarArea, t);
	cadPToolBar = t;

    cadToolBar = new QG_CadToolBar(t, "CAD Tools");
    cadToolBar->createSubToolBars(actionHandler);
    connect(cadToolBar, SIGNAL(signalBack()),this, SLOT(slotBack()));
	
	//harrison
	//Alert toolbar left
    /*
	sensorToolBar= new QToolBar("Sensor Tools", this);
	sensorToolBar->setSizePolicy(toolBarPolicy);
	sensorToolBar->setObjectName("SENSOR");
    sensor2ToolBar= new QToolBar("Sensor2 Tools", this);
	sensor2ToolBar->setSizePolicy(toolBarPolicy);
	sensor2ToolBar->setObjectName("SENSOR2");
	*/
    for(int i=0;i<NUM_SENSORTOOLBAR;i++){
        sensorToolBar[i] = new QToolBar("Sensor", this);
        sensorToolBar[i]->setIconSize(QSize(56,56));
        sensorToolBar[i]->setObjectName("SENSOR");
        sensorToolBar[i]->setOrientation(Qt::Vertical);
        sensorToolBar[i]->setFloatable(false);
        sensorToolBar[i]->setStyleSheet("QToolButton:!hover {background-color:white;border:1px solid gray} QToolBar {background: white;}");
    }
...
//harrison-ADD
void QC_ApplicationWindow::activateMapWindow(const QString& mapFile) {
    ...
    for(int i=0; i<windows.size();i++){
		w = windows.at(i);
        if(w->windowTitle().contains(mapFile)){
			//qDebug() << "active:" << mapFile;
			
			mdiAreaCAD->setActiveSubWindow(w);
	        w->raise();
	        w->showMaximized();
	        w->setFocus();
	       	//setWindowTitle("SCADA - " + mapFile);
			break;
		}
	}
...
void QC_ApplicationWindow::setSvModeView(double colorImage) {
	...
	for(int i=0;i<windows.size();i++){
		...
		qobject_cast<QC_MDIWindow*>(m->widget())->setImageColor(colorImage);
	}
        ...
        slotFileOpen(const QString& fileName, RS2::FormatType type)
{
    ...
    if (!fileName.isEmpty())
         {
        //qDebug() <<"QC_ApplicationWindow::slotFileOpen: creating new doc window";
        if (openedFiles.indexOf(fileName) >=0) {
            //QString message=tr("Warning: File already opened : ")+fileName;
            //commandWidget->appendHistory(message);
            //statusBar()->showMessage(message, 2000);
            //qDebug() << "slotFileOpen:already opened:" << fileName;
            QApplication::restoreOverrideCursor();
			return;
        }
        // Create new document window:
        ...
        //qDebug() <<"QC_ApplicationWindow::slotFileOpen: linking layer list";
        // link the layer widget to the new document:
        layerWidget->setLayerList(w->getDocument()->getLayerList(), false);
        // link the block widget to the new document:
        blockWidget->setBlockList(w->getDocument()->getBlockList());
        ...
        //qDebug() <<"QC_ApplicationWindow::slotFileOpen: open file";
        ...
        int dockWidth = DOCK_WIDTH;
        int dockHeight = DOCK_WIDTH * 2600/3600;
     
        /*20191013 disable minimap
        minimapDock = new QDockWidget(this);
        minimapDock->setFloating(false);
        minimapDock->setFeatures(QDockWidget::NoDockWidgetFeatures);
        minimapDock->setFixedSize(dockWidth,dockHeight);
        minimapDock->setWindowTitle(tr("迷你地圖"));

        mMiniMap = new MiniMap(minimapDock);
        mMiniMap->setMap(w);
        mMiniMap->setFixedSize(dockWidth, dockHeight);
     
        minimapDock->setWidget(mMiniMap);
        ...
        */
        ...
        if(sensordock == true){
            ...
            multiWidget->setFixedSize(dockWidth,600);
            multiWidget->setStyleSheet("background-color:white;");
            for(int i=0;i<NUM_SENSORTOOLBAR;i++){
                layout->addWidget(sensorToolBar[i]);
            }
            multiWidget->setLayout(layout);
            sensorDock = new QDockWidget(this);
            sensorDock->setFloating(false);
            sensorDock->setFeatures(QDockWidget::NoDockWidgetFeatures);
            
            sensorDock->setWindowTitle(tr("感測器"));
            sensorDock->setWidget(multiWidget);
            addDockWidget (Qt::RightDockWidgetArea, sensorDock);
            sensor2Dock = new QDockWidget(this);
            sensor2Dock->setFloating(false);
            sensor2Dock->setFeatures(QDockWidget::NoDockWidgetFeatures);
            sensor2Dock->setMaximumWidth(dockWidth);
            QWidget* wig =  new QWidget(this);
            wig->setFixedSize(dockWidth,400);
            sensor2Dock->setWidget(wig);
            addDockWidget (Qt::RightDockWidgetArea, sensor2Dock);
        }
        //////////////////////////////////////////////////////////
        ...
        //qDebug() <<"QC_ApplicationWindow::slotFileOpen: update recent file menu: 2";
        updateRecentFilesMenu();

  
        //qDebug() <<"QC_ApplicationWindow::slotFileOpen: set caption";
        ...
        //qDebug() <<"QC_ApplicationWindow::slotFileOpen: set caption: OK";

        //qDebug() <<"QC_ApplicationWindow::slotFileOpen: update coordinate widget";
        // update coordinate widget format:
        RS_DIALOGFACTORY->updateCoordinateWidget(RS_Vector(0.0,0.0),
                RS_Vector(0.0,0.0),
                true);
        //qDebug() <<"QC_ApplicationWindow::slotFileOpen: update coordinate widget: OK";
    ...
    //qDebug() <<"QC_ApplicationWindow::slotFileOpen(..) OK";
}

void QC_ApplicationWindow::setSvMode(double colorImage,bool alert, QListWidget* widget)
{
	
	if(alert == false){
		QTimer *timer = new QTimer(this);
	    connect(timer, SIGNAL(timeout()), this, SLOT(slotSetSvMode()));
	    timer->start(3000);
	}else
	if(alert == true){
		QTimer *timer = new QTimer(this);
	    connect(timer, SIGNAL(timeout()), this, SLOT(slotSetSvAlertMode()));
	    timer->start(3000);
		//
	}
	...
	if(m != NULL){
		...
		m->setImageColor(colorImage);
	}*/
	
	setSvModeView(colorImage);

    addEventDock(widget);
}

void QC_ApplicationWindow::addEventDock(QListWidget* widget)
{
    if(widget == nullptr)
        return;

    eventDock = new QDockWidget(this);
    eventDock->setStyleSheet("background-color:rgb(1,37,44);");
    eventDock->setFloating(true);
    eventDock->setFeatures(QDockWidget::DockWidgetClosable | QDockWidget::DockWidgetFloatable | QDockWidget::DockWidgetMovable);
    //eventDock->setFeatures(QDockWidget::DockWidgetClosable | QDockWidget::DockWidgetFloatable );
    eventDock->setWindowTitle("事件列表");
    //eventDock->setFixedSize(400,500);
    eventDock->setMinimumWidth(550);
    eventDock->setMinimumHeight(650);
    eventDock->setAttribute(Qt::WA_DeleteOnClose);
    eventList = new QListWidget(eventDock);
    eventList->setStyleSheet("background-color:rgb(1,37,44);");
    eventList->setFont(QFont("Microsoft YaHei UI", 12, QFont::Normal));
    //eventList->setWordWrap(true);
    eventList->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
    eventDock->setWidget(eventList);
    addDockWidget(Qt::TopDockWidgetArea, eventDock);
    eventDock->setFloating(true);

    for(int i=0;i<widget->count();i++){
        QListWidgetItem *pItem = widget->item(i);
        if(pItem == nullptr)
            continue;

        QListWidgetItem *pItemNew = new QListWidgetItem();
        pItemNew->setForeground(pItem->foreground());
        pItemNew->setText(pItem->text());
        pItemNew->setWhatsThis(pItem->whatsThis());
        QSize size = pItem->sizeHint();
        size.setHeight(20);
        pItemNew->setSizeHint(size);
        eventList->addItem(pItemNew);
    }
    QRect screenGeometry = QApplication::desktop()->availableGeometry();
    resize(screenGeometry.width(), screenGeometry.height());
    eventDock->move(screenGeometry.width()-550, screenGeometry.height()-650);
}


void QC_ApplicationWindow::setEdMode()
{
	svModeStatus = false;
    for(int i=0;i<NUM_SENSORTOOLBAR;i++){
        sensorToolBar[i]->show();
    }
	//sensorToolBar->show();
	//sensor2ToolBar->show();
	modifyToolBar->show();
...
void QC_ApplicationWindow::slotSetSvMode()
{
	//view mode
	//zoomActionList = zoomToolBar->actions();
	//zoomActionList[6]->trigger();
	modifyAction->trigger();
}

void QC_ApplicationWindow::slotSetSvAlertMode()
{
	//modify mode
	modifyAction->trigger();
}
...
//print alerts
void QC_ApplicationWindow::slotFilePrint(QMap<QString,double>& rtuList) {
    qDebug() << "QC_ApplicationWindow::slotFilePrint.";

	QMap<QString,double> rtuAlertFire = rtuList;
...
//    qDebug()<<"paper size=("<<printer.paperSize(QPrinter::Millimeter).width()<<", "<<printer.paperSize(QPrinter::Millimeter).height()<<")";
	...
	if(!pInfo.isNull()){
		QString printerName = pInfo.printerName();
		printer.setPrinterName(printerName);
	}
    QPrintDialog printDialog(&printer, this);
    printDialog.setOption(QAbstractPrintDialog::None);
    ...
    if (bStartPrinting) {
		...
		//qDebug() << "printW:" << printer.width();
		//qDebug() << "printH:" << printer.height();
        ...
        RS_GraphicView *gv1 = getGraphicView();
		gv1->setPrinting(true);

		int gw = gv1->getWidth();
		int gh = gv1->getHeight();
		//qDebug() << "gw:gh:" << gw <<":" <<gh; 
		
        RS_Vector fact = gv1->getFactor();
		int ox = gv1->getOffsetX();
		int oy = gv1->getOffsetY();
        //qDebug() << "offsetX:" << ox ;
        //qDebug() << "offsetY:" << oy ;
		double ratio = printer.width()/gw;
		gv1->setFactor(ratio/2);
        gv1->setOffset(1,0-ratio*gh);
		
		gv1->drawLayer1(&painter);
		gv1->drawLayer3(&painter);
		gv1->drawLayer8(&painter,rtuAlertFire);
        
        //gv.drawEntity(&painter, graphic);

        // GraphicView deletes painter
        painter.end();
        gv1->setFactor(fact.x);
		gv1->setOffset(ox,oy);
		gv1->setPrinting(false);
...
//    qDebug()<<"paper size=("<<printer.paperSize(QPrinter::Millimeter).width()<<", "<<printer.paperSize(QPrinter::Millimeter).height()<<")";
    ...
    if (bStartPrinting) {
		...
		qDebug() << "setFactor:" << f*scale;

		gv.setContainer(graphic);
		*/

		/*
        gv1->setBackground(RS_Color(255,255,255));
		QC_MDIWindow* w = getMDIWindow();
		if(w !=NULL)
			RS_Graphic* graphic = w->getDocument()->getGraphic();
...
void QC_ApplicationWindow::slotPrintMap(){
	RS_GraphicView *gv1 = getGraphicView();
}
...
void QC_ApplicationWindow::slotFilePrintPreview(bool on) {
    ...
    // open print preview:
        ...
        if (ppv!=NULL) {
            qDebug() << "QC_ApplicationWindow::slotFilePrintPreview(): show existing";
...
//            std::cout<<"QC_ApplicationWindow::slotFilePrintPreview(bool on): emit(printPreviewChanged(true))"<<std::endl;
            ...
            if (!parent->getGraphicView()->isPrintPreview()) {
                ...
                qDebug() << "QC_ApplicationWindow::slotFilePrintPreview(): create";
...
void QC_ApplicationWindow::slotSensor(int i){
	alertImageFile = sensorPrefixPath + QString::number(i,10) + sensorSuffixPath;;
...
void QC_ApplicationWindow::startAlert(QMap<QString,double>& rtus)
{
	//qDebug() << "QC_ApplicationWindow:startAlert.";
	//test block,set rtu list
	/*
	QString addr;
	addr = "00010041";
	alertRTUs.append(addr);
    addr = "00010074";
	alertRTUs.append(addr);
	*/
	alertRTUs = rtus;
	QC_MDIWindow* m = getMDIWindow();
	if(m != NULL){
		//qDebug() << "QC_ApplicationWindow:startAlert:mdi.";
		m->enableAlarm(alertRTUs);
	}
}

void QC_ApplicationWindow::stopAlert()
{
	QC_MDIWindow* m = getMDIWindow();
	if(m != NULL){
		m->disableAlarm();
...
/*
void QC_ApplicationWindow::startAlertR4(QMap<String,double>& rtus)
{
	//qDebug() << "QC_ApplicationWindow:startAlert.";
	//test block,set rtu list
	...
	if(m != NULL){
		//qDebug() << "QC_ApplicationWindow:startAlert:mdi.";
		m->enableAlarmR4(alertRTUs);
	}
}

void QC_ApplicationWindow::stopAlertR4()
{
	QC_MDIWindow* m = getMDIWindow();
	if(m != NULL){
		m->disableAlarmR4();
...
void QC_ApplicationWindow::startAlertFault(QStringList& rtus)
{
	//qDebug() << "QC_ApplicationWindow:startAlert.";
	//test block,set rtu list
	...
	if(m != NULL){
		//qDebug() << "QC_ApplicationWindow:startAlert:mdi.";
		m->enableAlarmFault(alertRTUs);
	}
}

void QC_ApplicationWindow::stopAlertFault()
{
	QC_MDIWindow* m = getMDIWindow();
	if(m != NULL){
		m->disableAlarmFault();
...
void QC_ApplicationWindow::setSvWindow(SV_Window *sv)
{
	svWindow = sv;
}


void QC_ApplicationWindow::setSvWindowActive()
{
	svWindow->setActive();
}

void QC_ApplicationWindow::setSensorDock(bool status)
{
    sensordock = status;
}
