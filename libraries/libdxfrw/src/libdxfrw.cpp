...
bool dxfRW::readRtuImages(DRW_Interface *interface_){
...
bool dxfRW::write(DRW_Interface *interface_, DRW::Version ver, bool bin){
    ...
    if (binary) {
    	...
    	//harrison
    	rtuFileName = fileName;
		int len = fileName.size();
		rtuFileName.replace(len-3,3,"rtu");
	    rtuOFilestream.open(rtuFileName.c_str(), std::ios_base::out | std::ios::trunc);
	...
	//harrison
	rtuOFilestream.flush();
	rtuOFilestream.close();
    return isOk;
}

bool dxfRW::writeRtuImages(DRW_Interface *interface_){
    bool isOk = false;
    std::ofstream filestr;
    binary = false;
    iface = interface_;
...
DRW_ImageDef* dxfRW::writeImage(DRW_Image *ent, std::string name){
    if (version > DRW::AC1009) {
		...
		writer->writeUtf8String(102, ent->location);
		writer->writeUtf8String(103, ent->rtuAddress);
		//harrison
