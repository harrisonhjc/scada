...
;General
  ...
  ;Name and file
  Name "${APPNAME}"
  OutFile "../../generated/SCADA-Installer.exe"

  ;Default installation folder
  InstallDir "$PROGRAMFILES\SCADA"

  ;Get installation folder from registry if available
  InstallDirRegKey HKCU "Software\SCADA" ""
...
Section "Install Section" SecInstall
  ...
  WriteRegStr HKCU "Software\SCADA" "" $INSTDIR

  ;Create uninstaller
  WriteUninstaller "$INSTDIR\Uninstall.exe"

  ; create shortcuts
  createShortCut "$DESKTOP\SCADA.lnk" "$INSTDIR\SCADA.exe"

  ; Startmenu shortcuts
  createDirectory "$SMPROGRAMS\SCADA\"
  createShortCut "$SMPROGRAMS\SCADA\SCADA.lnk" "$INSTDIR\SCADA.exe"
  createShortCut "$SMPROGRAMS\SCADA\Uninstall.lnk" "$INSTDIR\Uninstall.exe"

  ; create add/remove software entries
  WriteRegStr HKLM "${UNINSTKEY}" "DisplayName" "${APPNAME}"
  WriteRegStr HKLM "${UNINSTKEY}" "DisplayIcon" "$INSTDIR\SCADA.exe"
  WriteRegStr HKLM "${UNINSTKEY}" "DisplayVersion" "${SCMREVISION}"
  WriteRegStr HKLM "${UNINSTKEY}" "Publisher" "SCADA"
  WriteRegStr HKLM "${UNINSTKEY}" "Version" "1.0"
  WriteRegStr HKLM "${UNINSTKEY}" "HelpLink" "http://"
  WriteRegStr HKLM "${UNINSTKEY}" "InstallLocation" "$INSTDIR"
  WriteRegStr HKLM "${UNINSTKEY}" "URLInfoAbout" "http://"
  WriteRegStr HKLM "${UNINSTKEY}" "Comments" "SCADA System"
  ...
  ; Open Donate URL
  ;Exec "rundll32 url.dll,FileProtocolHandler http://"
...
Section "Uninstall"
  ...
  Delete "$DESKTOP\SCADA.lnk"
  RMDir /r "$SMPROGRAMS\SCADA\"
  RMDir /r $INSTDIR

  RMDir "$INSTDIR"

  DeleteRegKey /ifempty HKCU "Software\SCADA"
